...
public class TetrisItemSlot : UIComponent, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    private WeaponAttributeData itemData;
    ...
    {
        Debug.Log(eventData.pointerCurrentRaycast.gameObject.GetComponentInParent<TetrisItemSlot>().itemData.Appearance.name);
    }
    ...
    public void InitItem(TetrisSlot slots, WeaponAttributeData data, Vector2 cellSize)
    {
        if (data == null) return;
        this.slots = slots;
    ...
    private void RescalingItem(RectTransform rect)
    {
        ...
        {
            child.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, itemData.MatrixData.ItemSize.y * child.rect.height);
            child.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, itemData.MatrixData.ItemSize.x * child.rect.width);

            foreach (RectTransform iconChild in child)
            {
                iconChild.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, itemData.MatrixData.ItemSize.y * iconChild.rect.height);
                iconChild.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, itemData.MatrixData.ItemSize.x * iconChild.rect.width);
